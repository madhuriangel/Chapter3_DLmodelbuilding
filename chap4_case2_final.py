...
# ---- cartopy (optional; script falls back if missing) ----
try:
...
# =========================
# CONFIG – CASE STUDY 2 (3-DAY WINDOW ONLY)
# =========================
START_DATE = np.datetime64("2023-06-20")
END_DATE   = np.datetime64("2023-06-22")   # 3 days: 20, 21, 22 June 2023
...
# Output
OUT_DIR = Path("CNN_LSTM_test/hindcast_compare_June20_2023_H3_only_TEST2")
...
def common_mask_dynamic(f: xr.DataArray, o: xr.DataArray):
    """Keep only where both forecast and obs are finite."""
...
def nearest_finite_series_pair(fore: xr.DataArray, obs: xr.DataArray, lat: float, lon: float, max_radius: int = 6):
    """
...
# Aggregate 3-day metrics
metrics_df = pd.DataFrame(rows).sort_values(["model"])
metrics_df.to_csv(OUT_DIR / "regional_metrics_h3_June_2023_3days.csv", index=False)
...
if len(h3_dates) > 0:
    ...
    daily_metrics_df.to_csv(OUT_DIR / "regional_metrics_h3_June_2023_daily.csv", index=False)
    print("\nPer-day regional metrics saved to: regional_metrics_h3_June_2023_daily.csv")
...
def plot_map_compact(da: xr.DataArray, title: str, path: Path):
    """Coastline overlay; labels on LEFT & BOTTOM only; compact frame."""
    if not HAS_CARTOPY:
    ...
    pm = ax.pcolormesh(dab.lon, dab.lat, dab.values, transform=proj,
...
print(" - regional_metrics_h3_June_2023_3days.csv")
print(" - regional_metrics_h3_June_2023_daily.csv")
